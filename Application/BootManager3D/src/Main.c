...
EFI_STATUS EFIAPI UefiMain(
  ...
  for (UINTN i = 0; i < TexturesCount; ++i) {
    INT32 Width, Height;
    Status = ReadBMP(SelfFSHandle, TexturesFiles[i], &Width, &Height, &Textures[i]);
  ...
  UINTN EntryToBoot = Game(Numbers, MapWidth, MapHeight, Textures, TexturesCount, BootEntries, BootEntriesCount);
