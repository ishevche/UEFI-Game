...
EFI_STATUS ParseSprite(IN CHAR16 *Filename, OUT EFI_GRAPHICS_OUTPUT_BLT_PIXEL **Pixels,
                       OUT UINTN *Width, OUT UINTN *Height) {
  EFI_STATUS Status = EFI_SUCCESS;
  CHAR8 * spriteData;
  UINTN sizeOfSprite;
  Status = ReadFile(SelfFSHandle, Filename, (VOID **) &spriteData, &sizeOfSprite);
  *Width = 0;
  while (spriteData[(*Width)] != '\n')
    (*Width)++;
  *Height = sizeOfSprite / ((*Width) + 1);
  *Pixels = AllocateZeroPool(sizeof(EFI_GRAPHICS_OUTPUT_BLT_PIXEL) * (*Width) * (*Height));
  for (INTN i = 0; i < (*Width); ++i) {
    for (INTN j = 0; j < (*Height); ++j) {
      switch (spriteData[j * ((*Width) + 1) + i]) {
        case 'k':
          (*Pixels)[j * (*Width) + i].Red = 0;
          (*Pixels)[j * (*Width) + i].Green = 0;
          (*Pixels)[j * (*Width) + i].Blue = 0;
          (*Pixels)[j * (*Width) + i].Reserved = 0xff;
          break;
        case 'b':
          (*Pixels)[j * (*Width) + i].Red = 47;
          (*Pixels)[j * (*Width) + i].Green = 54;
          (*Pixels)[j * (*Width) + i].Blue = 153;
          (*Pixels)[j * (*Width) + i].Reserved = 0xff;
          break;
        case 'l':
          (*Pixels)[j * (*Width) + i].Red = 77;
          (*Pixels)[j * (*Width) + i].Green = 109;
          (*Pixels)[j * (*Width) + i].Blue = 243;
          (*Pixels)[j * (*Width) + i].Reserved = 0xff;
          break;
        case 'y':
          (*Pixels)[j * (*Width) + i].Red = 255;
          (*Pixels)[j * (*Width) + i].Green = 242;
          (*Pixels)[j * (*Width) + i].Blue = 0;
          (*Pixels)[j * (*Width) + i].Reserved = 0xff;
          break;
        case 'g':
          (*Pixels)[j * (*Width) + i].Red = 255;
          (*Pixels)[j * (*Width) + i].Green = 194;
          (*Pixels)[j * (*Width) + i].Blue = 14;
          (*Pixels)[j * (*Width) + i].Reserved = 0xff;
          break;
        case 'r':
          (*Pixels)[j * (*Width) + i].Red = 180;
          (*Pixels)[j * (*Width) + i].Green = 180;
          (*Pixels)[j * (*Width) + i].Blue = 180;
          (*Pixels)[j * (*Width) + i].Reserved = 0xff;
          break;
        case 'w':
          (*Pixels)[j * (*Width) + i].Red = 255;
          (*Pixels)[j * (*Width) + i].Green = 255;
          (*Pixels)[j * (*Width) + i].Blue = 255;
          (*Pixels)[j * (*Width) + i].Reserved = 0xff;
          break;
        case 'p':
          (*Pixels)[j * (*Width) + i].Red = 255;
          (*Pixels)[j * (*Width) + i].Green = 205;
          (*Pixels)[j * (*Width) + i].Blue = 148;
          (*Pixels)[j * (*Width) + i].Reserved = 0xff;
          break;
        case 's':
          (*Pixels)[j * (*Width) + i].Red = 255;
          (*Pixels)[j * (*Width) + i].Green = 224;
          (*Pixels)[j * (*Width) + i].Blue = 189;
          (*Pixels)[j * (*Width) + i].Reserved = 0xff;
          break;
        case 'n':
          (*Pixels)[j * (*Width) + i].Red = 156;
          (*Pixels)[j * (*Width) + i].Green = 90;
          (*Pixels)[j * (*Width) + i].Blue = 60;
          (*Pixels)[j * (*Width) + i].Reserved = 0xff;
          break;
        case 'd':
          (*Pixels)[j * (*Width) + i].Red = 107;
          (*Pixels)[j * (*Width) + i].Green = 37;
          (*Pixels)[j * (*Width) + i].Blue = 7;
          (*Pixels)[j * (*Width) + i].Reserved = 0xff;
          break;
        case '.':
        default:
